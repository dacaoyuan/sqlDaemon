...
import com.everyoo.localdata.sql.helper.MyDatabaseHelper;
...
public class MyDataDao implements MyDataImpl {
    ...
    /**
     * 注意要 判断表是否存在，在修改，否则报错
     *
     * @param newTable
     * @param oldTable
     * @return
     */
    public int modifyTables(String newTable, String oldTable) {
        int number = -1;
        try {
            if (dao.getTableName().equals("Book")) {
                number = dao.executeRaw("alter table Book rename to Book1");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        System.out.println("MyDataDao.modifyTables TableName=" + dao.getTableName());
        return number;
    }


    public int deleteTables(String table) {
        int number = -1;
        try {
            number = dao.executeRaw("drop table " + table);
        } catch (SQLException e) {
            e.printStackTrace();
        }
